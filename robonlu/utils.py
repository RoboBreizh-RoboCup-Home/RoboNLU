...
def compute_metrics(intent_preds,intent_labels,slot_preds, slot_labels):
    ...
    sementic_result = overall(intent_preds, intent_labels, slot_preds, slot_labels)
    # sementic_result = get_sentence_frame_acc(intent_preds, intent_labels, slot_preds, slot_labels)
...
def compute_metrics_final(slot_preds,
                                 slot_labels,
                                 intent_token_preds=None,
                                 intent_tokens=None,
                                 referee_token_preds_list = None,
                                 out_referee_token_list = None):


    intent_seq_existence = (intent_token_preds != None and intent_tokens != None)


    pro_existence = (referee_token_preds_list != None and out_referee_token_list != None) #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    if pro_existence:
        assert len(referee_token_preds_list) == len(out_referee_token_list) #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    if intent_seq_existence:
        assert  len(slot_preds) \
               == len(slot_labels) == len(intent_token_preds) == len(intent_tokens), 'the length should be the same'
    ...
    pro_result = get_pro_metrics(referee_token_preds_list,out_referee_token_list)
    ...
    # results.update(intent_result)
    results.update(slot_result)
    results.update(pro_result)
    if intent_seq_existence:
        results.update(intent_token_result)
    # if tag_intent_existence:    # results.update(sementic_result)
    #     results.update(tag_intent_result)

    return results


def compute_metrics_multi_intent_Pro(intent_preds,
                                 intent_labels,
                                 slot_preds,
                                 slot_labels,
                                 intent_token_preds=None,
...
def get_pro_metrics(preds, labels):
    ...
    assert len(preds) == len(labels)
    return {
        "Pro_precision": precision_score(labels, preds),
        "Pro_recall": recall_score(labels, preds),
        "Pro_f1": f1_score(labels, preds, average='micro')
    }
...
def overall(intent_preds, intent_labels, slot_preds, slot_labels):
    ...
    for preds, labels in zip(slot_preds, slot_labels):
        ...
        for p, l in zip(preds, labels):
            if p != l:
                ...
                break
        slot_result.append(int(one_sent_result))
    ...
    for preds, labels in zip(intent_preds, intent_labels):
        ...
        for p, l in zip(preds, labels):
            if p != l:
                one_sent_result = False
                break
        intent_result.append(int(one_sent_result))
