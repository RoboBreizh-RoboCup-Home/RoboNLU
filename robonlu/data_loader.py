...
class InputExampleMultiIntent_Pro(InputExample):
    def __init__(self,
        ...
        self.BI_tag_mask = BI_tag_mask
        self.tag_intent_label = tag_intent_label
        self.referee_labels = referee_labels #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
class InputFeaturesMultiIntent_Pro(InputFeatures):
    def __init__(self,
        ...
        self.referee_labels_ids = referee_labels_ids
##############################################################################################################
        ...
        for i, (text, intent, slot, intent_token) in enumerate(zip(texts, intents, slots, intent_tokens)):
            ...
            try:
                for idx, tag in enumerate(entities):
                    ...
                    assert tag_intent_label[idx] != self.intent_labels.index("O"), 'The intent tagged is O!'
            except:
                logger.info('Error')
                logger.info(text)
    ...
    def _read_file(cls, input_file, quotechar=None):
        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            for line in f:
                lines.append(line.strip())
            return lines
    def _create_examples(self, texts, intents, slots, intent_tokens, pro_tokens,set_type):
        """
        Creates examples for the training and dev sets.
        ...
        """
        examples = []
        for i, (text, intent, slot, intent_token,pro_token) in enumerate(zip(texts, intents, slots, intent_tokens, pro_tokens)):

            # if i == 0:
            #     print('joint process',text)
            #     print('len pro_token: ',len(pro_token.split()))
            # train-i
            guid = "%s-%s" % (set_type, i)
            # 1. input_text
            words = text.split()  # Some are spaced twice
            ...
            #-------- debug ----------
            # for int_tok in intent.split('#'):
            #     if int_tok not in self.intent_labels:
            #         print(int_tok)
            #--------------------------
            intent_label_token = [self.intent_labels.index(int_tok) if int_tok in self.intent_labels else self.intent_labels.index('UNK') for int_tok in intent.split('#')]
            ...
            #---------------------------- Pro labels -----------------------------------
            pro_labels = []
            for lab in pro_token.split():#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
                pro_labels.append(int(1) if lab == 'referral'  else int(0))

            referee_labels = []
            for lab in pro_token.split():#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
                # if i == 0:
                #     print(lab)
                if lab == 'O' or lab == 'referral':
                    referee_labels.append(1)
                elif lab == 'B-referee':
                    referee_labels.append(2)
                elif lab == 'PAD':
                    referee_labels.append(0)

            # if i == 0:
            #     print('referee_labels: ',referee_labels)
...
#                     tag_intent_label[idx] != self.intent_labels.index("O"), 'The intent tagged is UNK or O!'
    ...
    def get_examples(self, mode):
        ...
        return self._create_examples(texts=self._read_file(os.path.join(data_path, self.input_text_file)),
                                     ...
                                     slots=self._read_file(os.path.join(data_path, self.slot_labels_file)),
                                     intent_tokens=self._read_file(os.path.join(data_path, self.intent_tokens_file)),
                                     pro_tokens = self._read_file(os.path.join(data_path, self.pro_token_file)),
                                     set_type=mode)
##############################################################################################################
...
def convert_examples_to_features_multi_Pro(examples, max_seq_len, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        #     print(example.slot_labels)
        #     print('len slot labels: ',len(example.slot_labels))
        #     print('len intent_tokens : ', len(example.intent_tokens))
        #     print('len B_pos_mask labels: ', len(example.B_tag_mask))
        #     print('len BI_pos_mask labels: ', len(example.BI_tag_mask))
        #     print('len pro_label labels: ', len(example.pro_labels))
        #     print('len referee_label labels: ', len(example.referee_labels))



        #-------------------------

        # print('example.pro_labels: ',example.pro_labels)
        for i,(word, slot_label, intent_token, B_pos_mask, BI_pos_mask, pro_label, referee_label) in enumerate(zip(
                example.words,
                example.slot_labels,
                example.intent_tokens,
                B_tag_mask,
            ...
            referee_labels_ids.extend([int(referee_label)] + [pad_token_label_id] * (len(word_tokens) - 1))#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if len(tokens) > max_seq_len - special_tokens_count:
            ...
            referee_labels_ids = referee_labels_ids[:(max_seq_len - special_tokens_count)]  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        referee_labels_ids += [pad_token_label_id]  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        referee_labels_ids = [pad_token_label_id] + referee_labels_ids  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        referee_labels_ids = referee_labels_ids + ([pad_token_label_id] * padding_length)  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        assert len(referee_labels_ids) == max_seq_len, "Error with slot labels length {} vs {}".format(len(referee_labels_ids),
                                                                                                   max_seq_len)  # !!!!!!!!!!!!!!!!!!!!!!!!!
        assert len(intent_tokens_ids) == max_seq_len, "Error with intent tokens length {} vs {}".format(len(intent_tokens_ids), max_seq_len)
        assert len(B_tag_mask_list) == max_seq_len, "Error with B_tag_mask_list length {} vs {}".format(len(B_tag_mask_list), max_seq_len)
        assert len(BI_tag_mask_list) == max_seq_len, "Error with BI_tag_mask_list length {} vs {}".format(len(BI_tag_mask_list), max_seq_len)
        ...
        if ex_index < 5:
            ...
            logger.info("referee_labels_ids: %s" % " ".join([str(x) for x in referee_labels_ids]))  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            logger.info("intent_tokens: %s" % " ".join([str(x) for x in intent_tokens_ids]))
        ...
        features.append(
            InputFeaturesMultiIntent_Pro(input_ids=input_ids,
                          ...
                          referee_labels_ids = referee_labels_ids
                          ))
    return features
##############################################################################################################
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if args.pro:
        ...
        all_referee_labels_ids = torch.tensor([f.referee_labels_ids for f in features],dtype=torch.long)  # !!!!!!!!!!!!!!!!!!!!!!
